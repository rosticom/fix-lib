...
//  For Searching Play List on YouTube
  Future<List> playlistItems(String query, {String type}) async {
    this.query = query;
    Uri url = api.playlistItems(query, type: type);
    var res = await http.get(url, headers: {"Accept": "application/json"});
    print ('url:');
    print (url);
    var jsonData = json.decode(res.body);
...
//    kind = data['id']['kind'].substring(8);
    kind = data['kind'].substring(8);
//    id = data['id'][data['id'].keys.elementAt(1)];
    id = data['id'];
//    print(data['id']);
    print('playlist item: '+id);
    url = getURL(kind, id);
